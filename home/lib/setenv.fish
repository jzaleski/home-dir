...
if string match -q -r  "^elixir" "$type"
  printf $type > $PWD/.exenv-version;
else if string match -q -r  "^node" "$type"
...
else if string match -q -r  "^sbt" "$type"
  printf $type > $PWD/.sbt-version;
else
